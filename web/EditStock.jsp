...
<form action="StockController" method="post">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="itemName" name="itemName" value="<%= stock.getItemName() %>" required>
    </div>
    <div class="mb-3">
        <label for="category" class="form-label">Category</label>
        <input type="text" class="form-control" id="category" name="category" value="<%= stock.getCategory() %>" required>
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="price" name="price" value="<%= stock.getPrice() %>" required>
    </div>
    <div class="mb-3">
        <label for="expdate" class="form-label">Expiry Date</label>
        <input type="date" class="form-control datepicker" id="expdate" name="expdate" value="<%= stock.getExpiryDate() %>" required>
    </div>
    <div class="mb-3">
        <label for="supplier" class="form-label">Supplier</label>
        <input type="text" class="form-control" id="supplier" name="supplier" value="<%= stock.getSupplier() %>" required>
    </div>
    <div class="mb-3">
        <label for="batchNumber" class="form-label">Batch Number</label>
        <input type="text" class="form-control" id="batchNumber" name="batchNumber" value="<%= stock.getBatchNumber() %>" required>
    </div>
    <div class="mb-3">
        <label for="location" class="form-label">Location</label>
        <input type="text" class="form-control" id="location" name="location" value="<%= stock.getLocation() %>" required>
    </div>
    <div class="mb-3">
        <label for="status" class="form-label">Status</label>
        <select class="form-select" id="status" name="status" required>
            <option value="Active" <%= stock.getStatus().equals("Active") ? "selected" : "" %>>Active</option>
            <option value="Discontinued" <%= stock.getStatus().equals("Discontinued") ? "selected" : "" %>>Discontinued</option>
            <option value="Out of stock" <%= stock.getStatus().equals("Out of stock") ? "selected" : "" %>>Out of stock</option>
        </select>
    </div>
...
function goBack() {
        ...
        var confirmation = confirm("Are you sure you want to cancel the editing and go back to the stock list page?");
        
        // If the user confirms, navigate back
        if (confirmation) {
            window.location.href = "ManageStock.jsp"; // Replace with the actual URL of your list page
        }
    }
    </script>
    </body>
